...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Suporte para ação de marcar fatura como paga via POST (compatibilidade)
    if (body && body.action === 'marcar_paga') {
      const id = body.id;
      if (!id) return NextResponse.json({ error: 'missing_id' }, { status: 400 });
      await query('UPDATE cant_funcionario_fatura SET status = ? WHERE id = ?', ['PAGA', id]);
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    console.error('PATCH /api/relatorios/funcionarios/faturas error', err);
    return NextResponse.json({ error: 'server_error' }, { status: 500 });
  }
}
