...
export async function GET(request: NextRequest) {
  try {
    ...
    if (fornecedor) {
      whereConditions.push('fornecedor LIKE ?');
    ...
    // Query principal
    const sql = `
      SELECT * FROM cant_view_conta_pagar_resumo
      ${whereClause}
    ...
    const countSql = `
      SELECT COUNT(*) as total FROM cant_view_conta_pagar_resumo
      ${whereClause}
    `;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await query(
      `
      INSERT INTO cant_conta_pagar (
        categoria_id, descricao, fornecedor, numero_documento, valor_original, 
    ...
    if (parcelas && parcelas > 1 && data_primeira_parcela) {
      await query(
        `
        CALL cant_sp_gerar_parcelas_conta_pagar(?, ?, ?)
      `,
