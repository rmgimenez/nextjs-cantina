...
export async function POST(request: NextRequest, { params }: any) {
  try {
    ...
    if (!valor_recebido || !data_recebimento || !forma_recebimento) {
    ...
    if (
      ![
        ...
        'CARTAO_CREDITO',
        'OUTRO',
      ].includes(forma_recebimento)
    ) {
      return NextResponse.json({ error: 'Forma de recebimento inválida' }, { status: 400 });
    }

    // Registra o recebimento usando a stored procedure
    await query(
      `
      CALL cant_sp_registrar_recebimento_conta(?, ?, ?, ?, ?, ?, ?, ?, ?)
    `,
      [
        id,
        parcela_id || null,
