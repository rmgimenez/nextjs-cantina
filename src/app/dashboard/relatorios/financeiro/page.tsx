...
export default function RelatorioFinanceiroPage() {
  ...
  const carregar = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/relatorios/gerenciais/performance?inicio=${inicio}&fim=${fim}`);
      if (res.ok) {
  ...
  return (
    <>
      <div className='bg-white border-bottom px-3 py-3'>
        <h1 className='h4 mb-1 text-dark'>Relatório Financeiro</h1>
        <p className='text-muted mb-0'>Performance por atendente</p>
      </div>
