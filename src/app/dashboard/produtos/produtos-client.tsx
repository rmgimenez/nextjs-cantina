...
export default function ProdutosClient({ initialProdutos, initialTipos }: Props) {
  ...
  async function handleCreateProduto(form: FormData) {
    ...
    const res = await fetch('/api/produtos', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ tipoId, nome, precoUnitario, codigoBarra, estoqueMinimo }),
    });
    if (res.ok) {
      reloadProdutos();
  ...
  function getStatusBadge(stock: number, estoqueMinimo?: number) {
    if (stock === 0) return <span className='badge bg-danger'>Sem Estoque</span>;
  ...
  return (
    <div className='space-y-4'>
      <div className='d-flex flex-wrap gap-2 align-items-center'>
        ...
        <Button variant='secondary' icon={<FiPlus />} onClick={() => setShowFormTipo(true)}>
          Novo Tipo
        </Button>
        <Button variant='primary' icon={<FiPlus />} onClick={() => setShowFormProduto(true)}>
          Novo Produto
        </Button>
        ...
        <div className='col-sm-6 col-md-3'>
          <Card>
            <CardContent className='pt-3'>
              ...
              <h5 className='mb-0'>
                {
                  optimisticProdutos.filter((p) => p.estoque_atual <= (p.estoque_minimo ?? 0))
                    .length
                }
              </h5>
            </CardContent>
          ...
          <Card>
            <CardContent className='pt-3'>
              ...
              <h5 className='mb-0'>
                R${' '}
                {optimisticProdutos
                  .reduce((acc, p) => acc + p.preco_unitario * p.estoque_atual, 0)
                  .toFixed(2)}
              </h5>
            </CardContent>
          </Card>
            ...
            <div className='modal-content'>
              <form action={handleCreateTipo}>
                ...
                <div className='modal-footer'>
                  <Button type='button' variant='outline' onClick={() => setShowFormTipo(false)}>
                    Cancelar
                  </Button>
              ...
              <form action={selectedProduto ? handleEditProduto : handleCreateProduto}>
                ...
                <div className='modal-body row g-3'>
                  <div className='col-md-6'>
                    <Input name='nome' label='Nome' defaultValue={selectedProduto?.nome} required />
                  </div>
            ...
            <div className='modal-content'>
              <form action={handleMovimentacao}>
                ...
                <div className='modal-body'>
                  ...
                  <Input type='number' step='0.001' name='quantidade' label='Quantidade' required />
                </div>
                <div className='modal-footer'>
                  <Button type='button' variant='outline' onClick={() => setShowFormMov(false)}>
                    Cancelar
                  </Button>
                  <Button type='submit' variant='primary'>
                    Lançar
