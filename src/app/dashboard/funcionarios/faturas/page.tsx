...
export default function FaturasFuncionariosPage() {
  ...
  const carregar = async () => {
    ...
    try {
      const res = await fetch(
        `/api/relatorios/funcionarios/faturas?ano=${ano}&mes=${mes}`
      );
      if (res.ok) {
  ...
  return (
    <DashboardLayout
      title="Faturas Funcionários"
      subtitle="Fechamento mensal de consumo"
    >
            ...
            <tbody>
              {faturas.map((f) => (
                <tr key={f.id}>
                  ...
                  <td>{f.status}</td>
                  <td>
                    {new Date(f.data_geracao).toLocaleDateString("pt-BR")}
                  </td>
                </tr>
              ))}
              {faturas.length === 0 && (
