...
export default function UsuariosClient() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.usuario.trim()) {
      ...
      errors.usuario = 'Nome de usuário deve conter apenas letras, números e sublinhado';
    }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errBody = await res.json().catch(() => ({ error: 'Erro desconhecido' }));
        addToast('error', 'Erro', errBody.error || 'Falha ao atualizar usuário');
        return;
      }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errBody = await res.json().catch(() => ({ error: 'Erro desconhecido' }));
        addToast('error', 'Erro', errBody.error || 'Falha ao alterar status do usuário');
        return;
      }
  ...
  async function searchAlunos() {
    if (!searchAluno || searchAluno.length < 3) {
      addToast('warning', 'Atenção', 'Digite pelo menos 3 caracteres para buscar');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.alunos && data.alunos.length > 0) {
          addToast('success', 'Sucesso', `Encontrados ${data.alunos.length} aluno(s)`);
        } else {
          addToast('info', 'Informação', 'Nenhum aluno encontrado com os critérios informados');
        }
      } else {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.funcionarios && data.funcionarios.length > 0) {
          addToast('success', 'Sucesso', `Encontrados ${data.funcionarios.length} funcionário(s)`);
        } else {
          addToast(
            'info',
            'Informação',
  ...
  async function salvarPrecoCargo() {
    ...
    try {
      ...
      if (!res.ok) {
        const errBody = await res.json().catch(() => ({ error: 'Erro desconhecido' }));
        addToast('error', 'Erro', errBody.error || 'Falha ao salvar preço do cargo');
        return;
      }
      ...
      addToast('success', 'Sucesso', `Preço para cargo "${payload.cargo}" salvo com sucesso!`);
    } catch (err) {
      console.error(err);
          ...
          <CardContent>
            <div className='row g-3'>
              ...
              <div className='col-md-3'>
                ...
                {formErrors.tipo && (
                  <div className='invalid-feedback d-block'>{formErrors.tipo}</div>
                )}
                  ...
                  <tbody>
                    {filteredUsuarios.length === 0 ? (
                      ...
                      filteredUsuarios.map((u) => (
                        <tr key={u.id} className={u.ativo ? '' : 'table-secondary'}>
                          <td>
                          ...
                          <td>
                            <div className='btn-group btn-group-sm'>
                              ...
                              >
                                <i
                                  className={`bi bi-${u.ativo ? 'person-dash' : 'person-check'}`}
                                ></i>
                              </Button>
                            </div>
                          </td>
                        ...
                        <td>
                          {f.valorRefeicao != null ? (
                            <span className='badge bg-success'>
                              {formatarMoeda(f.valorRefeicao)}
                            </span>
                          ) : (
                            <span className='badge bg-warning text-dark'>Não definido</span>
                          )}
                        </td>
                      </tr>
                    ))
                      ...
                      <tr key={p.id}>
                        ...
                        <td>
                          <span className='badge bg-success fs-6'>
                            {formatarMoeda(p.valor_refeicao)}
                          </span>
                        </td>
                        <td>
                          {p.ativo ? (
  ...
  return (
    <div>
      ...
      <ConfirmModal
        ...
        title={deleteModal.usuario?.ativo ? 'Desativar Usuário' : 'Ativar Usuário'}
        message={
          deleteModal.usuario
