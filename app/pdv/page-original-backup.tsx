...
export default function PDVPage() {
  ...
  const [contaFuncionarioInfo, setContaFuncionarioInfo] =
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const res = await fetch(
        `/api/alunos/busca?q=${encodeURIComponent(q)}&limit=10`
      );
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const res = await fetch(
        `/api/funcionarios/busca?q=${encodeURIComponent(q)}&limit=10`
      );
      const d = await res.json();
  ...
  async function selecionarAluno(a: AlunoConta) {
    try {
      const res = await fetch(
        `/api/alunos/contas/${encodeURIComponent(String(a.ra))}`
      );
      const d = await res.json();
      if (d?.data) {
        setAluno(d.data);
  ...
  async function carregarContaFuncionario(codigo: number, cargo?: string) {
    ...
    try {
      const res = await fetch(
        `/api/funcionarios/consumo?codigo_funcionario=${encodeURIComponent(
          String(codigo)
        )}&limit=5`
      );
      if (res.ok) {
  ...
  function addItem(p: Produto) {
    setItens((cur) => {
      ...
      return [
        ...
        {
          id_produto: p.id,
          quantidade: p.por_quilo ? 1 : 1,
          peso: p.por_quilo ? 0.1 : undefined,
        },
      ];
    });
  }
    ...
    setItens((cur) =>
      cur.map((i) =>
        i.id_produto === id_produto
          ? {
              ...i,
              [field]: field === "quantidade" ? Number(value) : Number(value),
            }
          : i
      )
    );
  ...
  function validarRestricoesVenda(): Array<{
    produto: Produto;
    restricao: any;
  }> {
        ...
        <div className={styles.headerBar}>
          <div className="d-flex justify-content-between align-items-center">
            <div>
              ...
              <small>
                Caixa:{" "}
                <strong>{statusCaixa?.aberto ? "ABERTO" : "FECHADO"}</strong>
                {statusCaixa?.aberto && (
                  <span className="ms-3">
                    Valor esperado: R${" "}
                    {Number(statusCaixa?.totais?.esperado || 0).toFixed(2)}
                  </span>
                )}
              </small>
            </div>
        ...
        {resumoVenda && (
          <div className="alert alert-success border-success mb-3">
            <div className="d-flex justify-content-between align-items-center">
              <div>
                <h5 className="mb-2">
                  ✓ Venda #{resumoVenda.id_venda} concluída!
                </h5>
                <div className="row g-2">
                  <div className="col-auto">
                    <strong>Total:</strong> R${" "}
                    {Number(resumoVenda.total).toFixed(2)}
                  </div>
                    ...
                    <div className="mt-3">
                      ...
                      {observacoes.map((obs) => {
                        ...
                        return (
                          <div
                            key={obs.id}
                            className={`${styles.obsAlert} ${alertClass}`}
                          >
                    ...
                    <div className="mt-3 border border-success rounded p-2 bg-light">
                      ...
                      {pacotesAluno.map((pacote) => {
                        ...
                        return (
                          <div
                            key={pacote.id}
                            className="small mb-2 pb-2 border-bottom"
                          >
                            <div className="fw-semibold">
                              {pacote.pacote_nome}
                            </div>
                            <div className="text-muted">
                              {getTipoLabel(pacote.tipo_refeicao)}
                            </div>
                            ...
                            {pacote.data_fim && (
                              ...
                              >
                                Válido até:{" "}
                                {new Date(pacote.data_fim).toLocaleDateString()}
                              </div>
                            )}
                          </div>
                        );
                    ...
                    <div className="border rounded p-2 mt-3 bg-light">
                      <div className="d-flex justify-content-between small mb-1">
                        ...
                        <strong>
                          {contaFuncionarioInfo.limite_credito !== null
                            ? `R$ ${Number(
                                contaFuncionarioInfo.limite_credito
                              ).toFixed(2)}`
                            : "Sem limite"}
                        </strong>
                      </div>
                      <div className="d-flex justify-content-between small mb-1">
                        ...
                        <strong className="text-warning">
                          R${" "}
                          {Number(
                            contaFuncionarioInfo.total_em_aberto || 0
                          ).toFixed(2)}
                        </strong>
                      </div>
                      {contaFuncionarioInfo.limite_disponivel !== null && (
                        <div className="d-flex justify-content-between small">
                          ...
                          >
                            R${" "}
                            {Number(
                              contaFuncionarioInfo.limite_disponivel
                            ).toFixed(2)}
                          </strong>
                        </div>
                      )}
              ...
              <div className={styles.produtosGrid}>
                {produtosFiltrados.length === 0 ? (
                  ...
                  produtosFiltrados.map((p) => {
                    ...
                    const temDesconto =
                      precoEspecial != null &&
                      Number(precoEspecial) !== Number(p.preco_venda);
                    const precoFinal = temDesconto
                      ? Number(precoEspecial)
                ...
                <div className="modal-body">
                  ...
                  <div className="mb-3">
                    <p className="fw-bold mb-2">
                      Este aluno possui as seguintes restrições:
                    </p>
                              ...
                              <div className="fw-bold mb-1">
                                {restricao.tipo_restricao === "PRODUTO" ? (
                                  <>
                                    ...
                                    Produto Restrito:{" "}
                                    {restricao.produto_nome || "N/A"}
                                  </>
                                ) : (
                                  <>
                                    <i className="bi bi-exclamation-circle text-warning me-2"></i>
                                    Tipo Restrito:{" "}
                                    {restricao.tipo_produto_nome || "N/A"}
                                  </>
                                )}
                              </div>
                              {restricao.motivo && (
                  ...
                  <div className="alert alert-info mb-0">
                    ...
                    <strong>Importante:</strong> O sistema bloqueará
                    automaticamente a venda de produtos restritos para este
                    aluno.
                  </div>
                </div>
            ...
            <div className="modal-dialog modal-dialog-centered modal-lg">
              <div className="modal-content border-danger shadow-lg">
                ...
                <div className="modal-body">
                  {aluno && (
                    <div className="alert alert-danger mb-4">
                      <div className="d-flex align-items-center">
                        <i
                          className="bi bi-person-fill-x me-3"
                          style={{ fontSize: "2rem" }}
                        ></i>
                        <div>
                  ...
                  <div className="alert alert-warning mb-3">
                    <i className="bi bi-exclamation-triangle-fill me-2"></i>
                    <strong>Atenção!</strong> A venda não pode ser concluída
                    porque o carrinho contém produtos restritos para este aluno.
                  </div>
                    ...
                    {produtosBloqueados.map(({ produto, restricao }, index) => (
                      ...
                      >
                        <div className="d-flex justify-content-between align-items-start">
                              ...
                              <div className="fw-semibold text-danger mb-1">
                                {restricao.tipo_restricao === "PRODUTO" ? (
                                  ...
                                  <>
                                    ...
                                    Restrição: Tipo de Produto (
                                    {restricao.tipo_produto_nome})
                                  </>
                                )}
                              </div>
                              {restricao.motivo && (
                          ...
                          <span className="badge bg-danger fs-6 ms-3">
                            {index + 1}
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="alert alert-info mb-0">
                    ...
                    <ul className="mb-0 mt-2">
                      ...
                      <li>
                        Ou cancele esta venda e inicie uma nova sem esses
                        produtos
                      </li>
                    </ul>
                  </div>
                ...
                <div className="modal-footer bg-light">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const idsBloqueados = produtosBloqueados.map(
                        (b) => b.produto.id
                      );
                      setItens((cur) =>
                        cur.filter((i) => !idsBloqueados.includes(i.id_produto))
                      );
                      setShowBloqueioVendaModal(false);
                      setProdutosBloqueados([]);
