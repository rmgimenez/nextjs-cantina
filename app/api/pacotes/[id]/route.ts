...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const pacotes = await query('SELECT * FROM cant_pacotes_alimentacao WHERE id = ?', [id]);

    if (!pacotes || pacotes.length === 0) {
      return NextResponse.json({ error: 'Pacote não encontrado' }, { status: 404 });
    }

    const pacote = pacotes[0] as any;
    const pacoteFormatado = {
      ...pacote,
      valor: parseFloat(pacote.valor),
      quantidade_refeicoes: parseInt(pacote.quantidade_refeicoes),
      validade_dias: pacote.validade_dias ? parseInt(pacote.validade_dias) : null,
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Validações
    if (tipo_refeicao) {
      const tiposValidos = ['LANCHE_MANHA', 'ALMOCO', 'LANCHE_TARDE', 'JANTAR', 'PERSONALIZADO'];
    ...
    const sql = `UPDATE cant_pacotes_alimentacao SET ${updates.join(', ')} WHERE id = ?`;
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await query('UPDATE cant_pacotes_alimentacao SET ativo = 0 WHERE id = ?', [id]);
    ...
    return NextResponse.json({ error: 'Erro ao inativar pacote' }, { status: 500 });
  }
}
