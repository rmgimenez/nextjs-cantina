...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existingFuncionario = await query('SELECT * FROM cant_usuarios_cantina WHERE id = ?', [
      id,
    ]);
    ...
    let updateFields = 'nome = ?, usuario = ?, email = ?, telefone = ?, id_perfil = ?, ativo = ?';
    const updateParams = [nome, usuario, email || null, telefone || null, id_perfil, ativo ? 1 : 0];
    ...
    await query(`UPDATE cant_usuarios_cantina SET ${updateFields} WHERE id = ?`, updateParams);
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await query('UPDATE cant_usuarios_cantina SET ativo = 0 WHERE id = ?', [id]);
