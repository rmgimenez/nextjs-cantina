...
export async function GET() {
  try {
    ...
    const vendasHojeRows = await query(
      `SELECT COALESCE(SUM(valor_total), 0) AS total, COUNT(*) AS qtd
       FROM cant_vendas
       WHERE DATE(dt_venda) = CURDATE() AND status = 'CONCLUIDA'`
    );
    const vendasHoje = vendasHojeRows[0] as { total: string | number; qtd: string | number };

    // Produtos ativos em estoque (contagem de produtos cadastrados ativos)
    const produtosAtivosRows = await query(
      `SELECT COUNT(*) AS total
       FROM cant_produtos p
       WHERE p.ativo = 1`
    );
    const produtosAtivos = (produtosAtivosRows[0] as { total: string | number }).total;

    // Alunos com contas ativas
    const alunosAtivosRows = await query(
      `SELECT COUNT(*) AS total FROM cant_contas_alunos WHERE ativo = 1`
    );
    const alunosAtivos = (alunosAtivosRows[0] as { total: string | number }).total;
    ...
    const alertas = (alertasRows[0] as { total: string | number }).total;
    ...
    const data = {
      ...
      ultimasVendas: (ultimasVendas as any[]).map((v) => ({
        id: v.id,
      ...
      estoqueBaixo: estoqueBaixo as any[],
    };
