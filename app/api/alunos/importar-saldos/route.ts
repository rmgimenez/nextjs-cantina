...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      !userRows ||
      userRows.length === 0 ||
      userRows[0].perfil_nome !== "ADMINISTRADOR"
    ) {
    ...
    const nomeArquivo =
      (formData.get("nomeArquivo") as string) || file?.name || "arquivo.csv";
    ...
    for (let i = 0; i < linhas.length; i++) {
      ...
      const alunoRows = (await query(
        "SELECT ra, nome FROM alunos WHERE ra = ?",
        [ra]
      )) as AlunoRow[];
      ...
      if (contaRows && contaRows.length > 0) {
        ...
        idConta = conta.id;
        saldoAnterior = Number(conta.saldo_atual);

        await query(
          "UPDATE cant_contas_alunos SET saldo_atual = ? WHERE id = ?",
          [saldo, idConta]
        );
    ...
    return NextResponse.json(
      { error: "Erro ao processar importação de saldos" },
      { status: 500 }
    );
  }
}
