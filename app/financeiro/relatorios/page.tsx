...
export default function RelatoriosInadimplenciaPage() {
  ...
  const [contasInadimplentes, setContasInadimplentes] = useState<ContaInadimplente[]>([]);
  ...
  const loadRelatorioInadimplencia = useCallback(async () => {
    try {
  ...
  }, [tipoFilter, diasAtrasoFilter]);

  useEffect(() => {
    if (user) {
      loadRelatorioInadimplencia();
    }
  }, [user, loadRelatorioInadimplencia]);
  ...
  const getCategoriaAtraso = (dias: number) => {
    if (dias <= 30) return { categoria: '1-30 dias', classe: 'bg-warning text-dark' };
          ...
          <div className='col-md-3'>
            <div className='card border-0 shadow-sm'>
              <div className='card-body text-center'>
                ...
                <h4 className='text-danger mb-0'>R$ {totais.totalValor.toFixed(2)}</h4>
              </div>
            </div>
          </div>
          <div className='col-md-3'>
            <div className='card border-0 shadow-sm'>
              <div className='card-body text-center'>
                ...
                <h4 className='text-danger mb-0'>{totais.porTipo.PAGAR.quantidade}</h4>
                <small className='text-muted'>R$ {totais.porTipo.PAGAR.valor.toFixed(2)}</small>
              </div>
            </div>
          </div>
          <div className='col-md-3'>
            <div className='card border-0 shadow-sm'>
              <div className='card-body text-center'>
                ...
                <h4 className='text-warning mb-0'>{totais.porTipo.RECEBER.quantidade}</h4>
                <small className='text-muted'>R$ {totais.porTipo.RECEBER.valor.toFixed(2)}</small>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className='row mb-4'>
          <div className='col-12'>
            <div className='card border-0 shadow-sm'>
              ...
              <div className='card-body'>
                <div className='row'>
                  {Object.entries(totais.porCategoria).map(([categoria, dados]) => (
                    <div key={categoria} className='col-md-3'>
                      <div className='text-center'>
                        ...
                        <p className='text-danger mb-0'>R$ {dados.valor.toFixed(2)}</p>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <tr>
                      <td colSpan={9} className='text-center py-4'>
                        <div className='empty-state'>
                          ...
                          <p className='text-muted'>Nenhuma conta inadimplente encontrada</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    contasInadimplentes.map((conta) => {
                      const categoriaAtraso = getCategoriaAtraso(conta.dias_atraso);
                      return (
                        <tr key={`${conta.tipo}-${conta.id}`}>
                          <td>
                            <span className={`badge ${getTipoBadge(conta.tipo)}`}>
                              {getTipoText(conta.tipo)}
                            </span>
                          </td>
                          <td className='fw-bold'>{conta.nome_cliente_fornecedor}</td>
                          <td>{conta.descricao}</td>
                          <td className='fw-bold text-danger'>R$ {conta.valor.toFixed(2)}</td>
                          ...
                          <td>
                            <span className='badge bg-secondary'>{conta.status}</span>
                          </td>
                        </tr>
                      );
                    })
