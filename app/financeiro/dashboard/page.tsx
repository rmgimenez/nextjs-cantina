      ...
      <div className='container-fluid'>
        ...
        <div className='d-flex justify-content-between align-items-center mb-4'>
          <div>
            ...
            <p className='text-muted'>Visão geral da saúde financeira da cantina</p>
          </div>
          ...
          <div className='col-md-3'>
            <div className='card border-0 shadow-sm'>
              <div className='card-body text-center'>
                ...
                <h4
                  className={`mb-0 ${
                    dashboardData.resumo.saldoLiquido >= 0 ? 'text-success' : 'text-danger'
                  }`}
                >
        ...
        <div className='row mb-4'>
            ...
            <div className='card border-0 shadow-sm'>
              ...
              <div className='card-body'>
                <div className='row text-center'>
                  {Object.entries(dashboardData.porStatus.receber).map(([status, valor]) => (
                    <div key={status} className='col-6 mb-3'>
                      <div className={`badge bg-${getStatusColor(status)} fs-6 p-2`}>
          ...
          <div className='col-md-6'>
            <div className='card border-0 shadow-sm'>
              ...
              <div className='card-body'>
                <div className='row text-center'>
                  {Object.entries(dashboardData.porStatus.pagar).map(([status, valor]) => (
                    <div key={status} className='col-6 mb-3'>
                      <div className={`badge bg-${getStatusColor(status)} fs-6 p-2`}>
                        {getStatusText(status)}
                      </div>
              ...
              <div className='card-body'>
                <div className='table-responsive'>
                  <table className='table table-hover'>
                    ...
                    <tbody>
                      {dashboardData.fluxoMensal.map((mes) => (
                        <tr key={mes.mes}>
                          <td className='fw-bold'>{mes.mes}</td>
                          <td className='text-success'>R$ {mes.receber.toFixed(2)}</td>
                          <td className='text-danger'>R$ {mes.pagar.toFixed(2)}</td>
                          <td
                            className={`fw-bold ${mes.saldo >= 0 ? 'text-success' : 'text-danger'}`}
                          >
                        ...
                        <tr>
                          <td colSpan={5} className='text-center py-4'>
                            <div className='empty-state'>
                              ...
                              <p className='text-muted'>Nenhuma transação recente</p>
                            </div>
                          </td>
                        </tr>
                      ) : (
                        dashboardData.ultimasTransacoes.map((transacao, index) => (
                          <tr key={index}>
                            <td>
                              <span
                                className={`badge ${
                                  transacao.tipo === 'RECEBER' ? 'bg-success' : 'bg-danger'
                                }`}
                              >
                                {transacao.tipo === 'RECEBER' ? 'Recebimento' : 'Pagamento'}
                              </span>
                            </td>
                            <td>{transacao.descricao}</td>
                            <td
                              className={`fw-bold ${
                                transacao.tipo === 'RECEBER' ? 'text-success' : 'text-danger'
                              }`}
                            >
                              R$ {transacao.valor.toFixed(2)}
                            </td>
                            <td>{new Date(transacao.dt_operacao).toLocaleDateString('pt-BR')}</td>
                            <td>
